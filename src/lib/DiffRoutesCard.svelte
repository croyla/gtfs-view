<script lang="ts">
  ...
  type RouteKind = RouteDiffKind | 'unchanged';

  const entries = $derived.by(() => {
    if (activeFilter === 'unchanged') {
      return [...compareData.routes.entries()]
        .filter(([id]) => !diff.routes.has(id) && gtfsData.routes.has(id))
        .map(([routeId, route]) => ({ routeId, diff: null as null, route, kind: 'unchanged' as RouteKind }))
        .sort((a, b) => (a.route?.route_short_name || a.routeId).localeCompare(b.route?.route_short_name || b.routeId));
    }
    return [...diff.routes.entries()]
      ...
      .map(([routeId, d]) => {
        ...
        return { routeId, diff: d, route, kind: d.kind as RouteKind };
      })
      .sort((a, b) => {
        const order: Record<RouteDiffKind, number> = { added: 0, removed: 1, changed: 2 };
        return order[a.kind as RouteDiffKind] - order[b.kind as RouteDiffKind];
      });
  });

  const BADGE: Record<RouteKind, string> = {
  ...
  const DOT: Record<RouteKind, string> = {
    added:     '#22c55e',
    removed:   '#ef4444',
    changed:   '#60a5fa',
    unchanged: '#ffffff',
...
{:else}
  <div class="space-y-1">
    {#each entries as { routeId, diff: d, route, kind } (routeId)}
      <button
      ...
      >
        <span class="h-2.5 w-2.5 shrink-0 rounded-full" style="background:{DOT[kind]}"></span>
        ...
        <span class="rounded px-1.5 py-0.5 text-[9px] font-semibold shrink-0 {BADGE[kind]}">
          {kind}
        </span>
      </button>
    {/each}
