...
>
  <div class="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
    <div class="flex items-start justify-between mb-1">
      <div>
        <h2 class="text-lg font-semibold text-white">Load comparison feed</h2>
        <p class="mt-0.5 text-sm text-slate-400">Upload a second GTFS .zip to diff against the loaded feed.</p>
      </div>
      <button
        class="ml-4 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-700 hover:text-slate-200 transition-colors"
        onclick={onClose}
        aria-label="Close"
      >
    ...
    <!-- Tabs -->
    <div class="my-5 flex gap-1 rounded-lg bg-slate-800 p-1">
