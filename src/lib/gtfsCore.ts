...
export type SerializedGtfsData = {
  agencies: [string, Agency][];
  stops: [string, Stop][];
  trips: [string, Trip][];
  routes: [string, Route][];
  shapes: [string, ShapePoint[]][];
  stopTimes: StopTime[];          // flat; main thread builds indices
...
): { tree: AgencyTreeNode[]; allShapeKeys: string[] } {
      ...
      for (const [key, groupTrips] of byShape) {
        ...
        if (isNoShape) {
          ...
          const headsigns = [...new Set(
            groupTrips.map(t => t.trip_headsign).filter((h): h is string => !!h),
          )];
        ...
        if (!seen.has(key)) { seen.add(key); allShapeKeys.push(key); }
      }
...
// ── Core parser (runs inside Web Worker) ──────────────────────────────────────

export function processGtfsBuffer(buffer: ArrayBuffer): SerializedGtfsData {
  ...
  for (const r of agenciesRaw) {
    ...
    agencies.set(id, {
      agency_id: id, agency_name: r.agency_name,
      agency_url: r.agency_url || undefined, agency_timezone: r.agency_timezone || undefined,
    });
  }
  ...
  for (const r of stopsRaw) {
    ...
    allStops.set(r.stop_id, {
      ...
      stop_code: r.stop_code || undefined, stop_desc: r.stop_desc || undefined,
    });
  }
  ...
  // Stop times (parse once, build referential sets)
  const stopTimes: StopTime[] = [];
  const referencedTripIds = new Set<string>();
  ...
  for (const r of stopTimesRaw) {
    ...
    referencedTripIds.add(r.trip_id);
  ...
  // tripStops index (sorted, deduplicated stop_ids per trip)
  ...
  // Synthetic shapes: generate polylines for trips without shapes.txt data
  ...
  // shapeColors (after synthetic IDs assigned)
