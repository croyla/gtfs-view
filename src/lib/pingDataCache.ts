...
const mem       = new Map<string, BlockPingData>();
...
export function pingCacheGet(blockId: string, date: string): BlockPingData | undefined {
  const mk = memKey(blockId, date);
  if (mem.has(mk)) return mem.get(mk)!;
  try {
    const raw = localStorage.getItem(lsKey(blockId, date));
    if (!raw) return undefined;
    const v = JSON.parse(raw) as BlockPingData;
    mem.set(mk, v);
    return v;
  } catch { return undefined; }
}

export function pingCacheSet(blockId: string, date: string, data: BlockPingData): void {
